...
object Anagrams extends AnagramsInterface:
  ...
  def wordOccurrences(w: Word): Occurrences =
    (w.toLowerCase.groupBy(c => c)).map((char, str) => (char, str.length)).toList.sorted
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] =
    occurrences.foldLeft(List[Occurrences](List()))( (acc, occPair) => {
      for
        accItem <- acc
        occCount <- 0 to occPair._2
      yield
        occCount match {
          case 0 => accItem
          case count => accItem :::List((occPair._1, count))
        }
    })
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences =
    val yMap = y.groupBy( item => item._1 ).transform((key, value) => value(0)._2)
    
    for
      xPair <- x
      if(xPair._2 - yMap.getOrElse(xPair._1, 0) > 0)
    yield
      (xPair._1, xPair._2 - yMap.getOrElse(xPair._1, 0))
